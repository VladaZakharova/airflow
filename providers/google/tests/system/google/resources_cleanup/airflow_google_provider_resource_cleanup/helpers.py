...
def get_resources_file(system_tests_project, resource_type: str | None = None, resource_file_path: Path | None = None):
    ...
    if labels:
        for label_key, label_value in labels.items():
            if label_key in c.DO_NOT_DELETE_LABELS or label_value in c.DO_NOT_DELETE_LABELS:
                if not silent:
                    print(f'The label: "{label_key}={label_value}" is protected for resource "{name}". Ignoring!')
                return False
...

        {
          ...
          "protected_resources": {
            ...
            "storage.googleapis.com/Bucket": [
              "//storage.googleapis.com/providers-dashboard-temp-logs-dev"
            ]
          }
        }
...
def get_tp_asset_data_item(project_id, asset_type="tp_vertex_ai_raycluster", config_path: Path | None = None):
...
def provide_tp_asset_resource_file(
    project_id,
    asset_type,
    ...
    dump_json(
        resources_file,
        [get_tp_asset_data_item(project_id, asset_type, config_path=config_path)],
    )
