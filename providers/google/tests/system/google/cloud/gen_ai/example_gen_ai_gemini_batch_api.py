...
REGION = "us-central1"
DAG_ID = "gen_ai_gemini_batch_api_dag"
...
UPLOADED_FILE_NAME = (
    "{{ task_instance.xcom_pull(task_ids='upload-file-for-batch-job-task', key='file_name') }}"
)
UPLOADED_EMBEDDINGS_FILE_NAME = (
    "{{ task_instance.xcom_pull(task_ids='upload-file-for-embeddings-batch-job-task', key='file_name') }}"
)

BATCH_JOB_WITH_INLINED_REQUESTS_NAME = (
    "{{ task_instance.xcom_pull(task_ids='create-batch-job-using-inlined-requests-task', key='job_name') }}"
)
BATCH_JOB_WITH_FILE_NAME = (
    "{{ task_instance.xcom_pull(task_ids='create-batch-job-using-file-task', key='job_name') }}"
)
EMBEDDINGS_BATCH_JOB_WITH_INLINED_REQUESTS_NAME = "{{ task_instance.xcom_pull(task_ids='create-embeddings-job-using-inlined-requests-task', key='job_name') }}"
EMBEDDINGS_BATCH_JOB_WITH_FILE_NAME = (
    "{{ task_instance.xcom_pull(task_ids='create-embeddings-job-using-file-task', key='job_name') }}"
)
...
with DAG(
    ...
    upload_file = GenAIGeminiUploadFileOperator(
        task_id="upload-file-for-batch-job-task",
    ...
    upload_embeddings_file = GenAIGeminiUploadFileOperator(
        task_id="upload-file-for-embeddings-batch-job-task",
        project_id=PROJECT_ID,
        location=REGION,
    ...
    get_file = GenAIGeminiGetFileOperator(
        task_id="get-file-for-batch-job-task",
        project_id=PROJECT_ID,
        location=REGION,
    ...
    list_files = GenAIGeminiListFilesOperator(
        task_id="list-files-files-api-task",
        project_id=PROJECT_ID,
        location=REGION,
        gemini_api_key=GEMINI_XCOM_API_KEY,
    )
    ...
    delete_file = GenAIGeminiDeleteFileOperator(
        task_id="delete-file-for-batch-job-task",
        project_id=PROJECT_ID,
        location=REGION,
        file_name=UPLOADED_FILE_NAME,
        gemini_api_key=GEMINI_XCOM_API_KEY,
    ...
    delete_embeddings_file = GenAIGeminiDeleteFileOperator(
        task_id="delete-file-for-embeddings-batch-job-task",
        project_id=PROJECT_ID,
        location=REGION,
    ...
    create_batch_job_using_inlined_requests = GenAIGeminiCreateBatchJobOperator(
        task_id="create-batch-job-using-inlined-requests-task",
        project_id=PROJECT_ID,
        location=REGION,
    ...
    create_batch_job_using_file = GenAIGeminiCreateBatchJobOperator(
        task_id="create-batch-job-using-file-task",
        project_id=PROJECT_ID,
        location=REGION,
        model="gemini-3-pro-preview",
        gemini_api_key=GEMINI_XCOM_API_KEY,
    ...
    create_embeddings_job_using_inlined_requests = GenAIGeminiCreateEmbeddingsBatchJobOperator(
        task_id="create-embeddings-job-using-inlined-requests-task",
        project_id=PROJECT_ID,
        location=REGION,
    ...
    create_embeddings_job_using_file = GenAIGeminiCreateEmbeddingsBatchJobOperator(
        task_id="create-embeddings-job-using-file-task",
        project_id=PROJECT_ID,
        location=REGION,
        model="gemini-embedding-001",
        wait_until_complete=False,
    ...
    delete_batch_job_1 = GenAIGeminiDeleteBatchJobOperator(
        task_id="delete-batch-job-1-task",
        project_id=PROJECT_ID,
        location=REGION,
        gemini_api_key=GEMINI_XCOM_API_KEY,
        job_name=BATCH_JOB_WITH_INLINED_REQUESTS_NAME,
    ...
    delete_batch_job_2 = GenAIGeminiDeleteBatchJobOperator(
        task_id="delete-batch-job-2-task",
        project_id=PROJECT_ID,
        location=REGION,
        gemini_api_key=GEMINI_XCOM_API_KEY,
        job_name=BATCH_JOB_WITH_FILE_NAME,
    ...
    delete_embeddings_batch_job_1 = GenAIGeminiDeleteBatchJobOperator(
        task_id="delete-embeddings-batch-job-1-task",
        project_id=PROJECT_ID,
        location=REGION,
        gemini_api_key=GEMINI_XCOM_API_KEY,
    ...
    delete_embeddings_batch_job_2 = GenAIGeminiDeleteBatchJobOperator(
        task_id="delete-embeddings-batch-job-2-task",
        project_id=PROJECT_ID,
        location=REGION,
        gemini_api_key=GEMINI_XCOM_API_KEY,
    ...
    delete_batch_job_1.trigger_rule = TriggerRule.ALL_DONE
    delete_batch_job_2.trigger_rule = TriggerRule.ALL_DONE
    delete_embeddings_batch_job_1.trigger_rule = TriggerRule.ALL_DONE
    delete_embeddings_batch_job_2.trigger_rule = TriggerRule.ALL_DONE
    delete_file.trigger_rule = TriggerRule.ALL_DONE
    delete_embeddings_file.trigger_rule = TriggerRule.ALL_DONE
    ...
    (
        ...
        >> [delete_file, delete_embeddings_file]
    )
