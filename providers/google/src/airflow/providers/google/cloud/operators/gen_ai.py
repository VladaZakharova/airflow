...
class GenAIGeminiCreateBatchJobOperator(GoogleCloudBaseOperator):
    ...
    :param retrieve_result: Optional. Put result to Xcom.
    :param check_job_interval: Optional. Interval to check job completion.
    :param gcp_conn_id: The connection ID to use connecting to Google Cloud.
    :param impersonation_chain: Optional service account to impersonate using short-term
        credentials, or chained list of accounts required to get the access_token
        of the last account in the list, which will be impersonated in the request.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.check_job_interval = check_job_interval
    ...
    def _wait_until_complete(self, job, check_interval: int = 30):
        try:
            while True:
                ...
                if job.state.name in [
                    ...
                    break
                self.log.info("Waiting for job execution, current state: %s", job.state.name)
            ...
            for inline_response in job.dest.inlined_responses:
                if inline_response.response:
                    ...
                    try:
                        ...
                        results.append(inline_response.response)  # Fallback
                elif inline_response.error:
                    self.log.warning("Error found in the inline results")
    ...
    def execute(self, context: Context):
        ...
        try:
            ...
            raise AirflowException("Something went wrong during creation of the batch job.")
        ...
        if self.wait_until_complete:
            job = self._wait_until_complete(job, self.check_job_interval)
...
class GenAIGeminiListBatchJobsOperator(GoogleCloudBaseOperator):
    """
    Get list of Batch jobs using Gemini API.
...
class GenAIGeminiDeleteBatchJobOperator(GoogleCloudBaseOperator):
    """
    Delete Batch job using Gemini API.
    ...
    def execute(self, context: Context):
        ...
        try:
            ...
            self.log.info("Job with name %s was successfully deleted.", self.job_name)
        except ValueError:
...
class GenAIGeminiCreateEmbeddingsBatchJobOperator(GoogleCloudBaseOperator):
    ...
    def execute(self, context: Context):
        ...
        if self.wait_until_complete:
            embeddings_job = self._wait_until_complete(embeddings_job, self.check_job_interval)

        return embeddings_job
...
class GenAIGeminiUploadFileOperator(GoogleCloudBaseOperator):
    ...
    def execute(self, context: Context):
        ...
        try:
            ...
            raise AirflowException("Error during file upload!")
...
class GenAIGeminiDeleteFileOperator(GoogleCloudBaseOperator):
    ...
    def execute(self, context: Context):
        ...
        try:
            self.hook.delete_file(file_name=self.file_name)
