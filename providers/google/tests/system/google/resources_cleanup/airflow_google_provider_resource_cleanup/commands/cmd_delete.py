...
from airflow_google_provider_resource_cleanup.handlers import DELETE_HANDLERS
from airflow_google_provider_resource_cleanup.helpers import (
    GCPProjectConfig,
    check_white_list,
    get_resources_file,
    get_tp_asset_data_item,
    load_json,
    provide_tp_asset_resource_file,
)
...
async def handle_asset_type(
    ...
    if asset_type not in constants.TENANT_PROJECT_ASSET_TYPES:
        print(f'{len(resources)} resources found for the asset type: "{asset_type}"!. Deleting...')
    else:
        print(f"Tenant project resource clean up requested: '{asset_type}'. Deleting attempt...")
...
async def handle_delete(
    ...
    provide_tp_asset_resource_file_fn=provide_tp_asset_resource_file,
    get_resources_file_fn=get_resources_file,
    load_json_fn=load_json,
    get_tp_asset_data_item_fn=get_tp_asset_data_item,
    ...
    if asset_type in constants.TENANT_PROJECT_ASSET_TYPES:
    ...
    if asset_type not in constants.TENANT_PROJECT_ASSET_TYPES:
        ...
        for _asset_type in constants.TENANT_PROJECT_ASSET_TYPES:
            _asset_type_pattern = constants.TENANT_PROJECT_ASSET_TYPES[_asset_type]
            _asset_type_value = ".".join(_asset_type_pattern.split(".")[:-1])
    ...
    if asset_type is not None:
        ...
        # append tenant project resource, that was not listed in the project resources file
        # tp_vertex_ai_raycluster
        categorized_resources["tp_vertex_ai_raycluster"] = [
            get_tp_asset_data_item_fn(project_id, asset_type="tp_vertex_ai_raycluster")
        ]
