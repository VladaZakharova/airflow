...
class GoogleBaseHook(BaseHook):
    ...
    def is_default_universe(self):
        global_universe_domain = os.getenv("GOOGLE_CLOUD_UNIVERSE_DOMAIN", None)
        if global_universe_domain == "googleapis.com" or global_universe_domain is None:
            return True
        return False
    ...
    def get_client_options(
        ...
        if api_endpoint_override and not self.is_default_universe():
            self.log.info(
                "Ignoring api_endpoint_override because the universe domain is not Google default universe."
            )

        if api_endpoint_override and self.is_default_universe():
