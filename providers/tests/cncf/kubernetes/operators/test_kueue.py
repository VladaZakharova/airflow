...
class TestKubernetesStartKueueJobOperator:
    ...
    @mock.patch(KUEUE_OPERATORS_PATH.format("warnings"))
    def test_init_suspend_is_none(self, mock_warnings):
        operator = KubernetesStartKueueJobOperator(
            task_id=TEST_TASK_ID,
        ...
        mock_warnings.warn.assert_called_once_with(
            "You have not set parameter `suspend` in class KubernetesStartKueueJobOperator. "
