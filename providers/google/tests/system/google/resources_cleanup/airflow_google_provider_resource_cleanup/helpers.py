...
class GCPProjectConfig:
    ...
    def load_from_file(cls, project_id, config_path: Path | None = None):
        ...
        if not file.exists():
            cfg = GCPProjectConfig(project_id=project_id)
        ...
        if unknown_keys:
            raise ValueError(
                f'Unexpected key(s) in project configuration "{file}": {", ".join(unknown_keys)}. '
                f"Supported keys are: {', '.join(sorted(allowed_keys))}."
            )

        config_data = data.copy()
        config_data.pop("project_id", None)  # The CLI project ID always wins over the config file value.
        return GCPProjectConfig(project_id=project_id, **config_data)
