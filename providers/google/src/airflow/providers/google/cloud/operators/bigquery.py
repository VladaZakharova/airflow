...
class BigQueryInsertJobOperator(GoogleCloudBaseOperator, _BigQueryInsertJobOperatorOpenLineageMixin):
    ...
    :param poll_interval: (Deferrable mode only) polling period in seconds to check for the status of job.
        Defaults to 4 seconds.
    :param submit_new_job_on_retry: In case of job failing with 429 rate limit errors it will be retried.
        Note: if this flag set to True then it is not possible to use force_rerun.
        Defaults to True.
    """
    ...
    def __init__(
        ...
        deferrable: bool = conf.getboolean("operators", "default_deferrable", fallback=False),
        poll_interval: float = 4.0,
        submit_new_job_on_retry: bool = True,
        **kwargs,
    ) -> None:
        super().__init__(**kwargs)
        ...
        self.deferrable = deferrable
        self.poll_interval = poll_interval
        self.submit_new_job_on_retry = submit_new_job_on_retry

        if self.submit_new_job_on_retry and self.force_rerun:
            raise AirflowException(
                "Both `force_rerun` and `submit_new_job_on_retry` are set to True. "
                "Only one of these flags can be set to True."
            )
    ...
    def execute(self, context: Any):
        ...
        ti_try_number = None
        if self.submit_new_job_on_retry and self.job_id is None and context["ti"].try_number > 2:
