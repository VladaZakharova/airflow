...
class TranslateDocumentOperator(GoogleCloudBaseOperator):
    ...
    :param enable_shadow_removal_native_pdf: Optional. If true, use the text removal server to remove the
        ...
        Shadow removal feature can only be enabled when both is_translate_native_pdf_only, pdf_native_only
        are False.
    ...
    def execute(self, context: Context) -> dict:
        ...
        try:
            self.log.info("Starting the document translation run")
            ...
            self.log.info("Document translation run complete")
        ...
        return type(doc_translation_result).to_dict(doc_translation_result)
